    ...
    m_currentSelectionOnly(false)
{
    ...
    pluginBox->setTitle(plugin->getType().c_str()); // JPMAUS  "Feature Extraction Plugin" !
    ...
    // JPMAUS add option to calculate Tempo
    _tempoChkBx = new QCheckBox(tr("Calculate Tempo"));
    connect(_tempoChkBx, &QCheckBox::toggled , this, [=](bool tick){m_tempoSelection=tick;});
    _tempoChkBx->setChecked(false);
    advancedLayout->addWidget(_tempoChkBx);
    // End
