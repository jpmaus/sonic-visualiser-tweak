...
#include <QComboBox>
#include "svcore/rdf/RDFTransformFactory.h"
#include <QFileDialog>
#include <QSpinBox>
using std::vector;
using std::map;
    ...
    m_templateWatcher(nullptr)
{
    ...
    // JPMAUS connect singals / slots for Compound Transform.
    connect(this, &MainWindow::startInnerTransform , this, &MainWindow::addInnerCalcsLayer, Qt::UniqueConnection);
    connect(this, &MainWindow::startCompoudTrasCals , this, &MainWindow::compoundTransCalcs, Qt::UniqueConnection);
...
MainWindow::setupFileMenu()
{
    ...
    menu->addAction(action);
    // JPMAUS new action import Image
    action = new QAction(tr("Import Image file..."), this);
    action->setStatusTip(tr("Import Audio From Image "));
    connect(action, SIGNAL(triggered()), this, SLOT(audio4rmImage()));
    //connect(this, SIGNAL(canExportLayer(bool)), action, SLOT(setEnabled(bool)));
    menu->addAction(action);
    // JPMAUS end
...
MainWindow::setupTransformsMenu()
{
    ...
    //
    // JPMAUS Add Annotator submenu under Transforms
    m_transformsMenu->addSeparator();
    m_rightButtonTransformsMenu->addSeparator();

    QMenu *annotatorMenu = m_transformsMenu->addMenu(tr("Annotator"));
    annotatorMenu->setTearOffEnabled(true);
    //
    // 1
    QAction *ant_Skl_Action = new QAction(tr("Generate a skeleton"), this);
    ant_Skl_Action->setStatusTip(tr("Generate a skeleton RDF transform file for transform id <I>, with default parameters for that transform, and write it to standard output."));
    ant_Skl_Action->setShortcut(tr("Ctrl+P"));
    connect(ant_Skl_Action, SIGNAL(triggered()), this, SLOT(genSkelton()));
    //connect(this, SIGNAL(canAddLayer(bool)), antrAction, SLOT(setEnabled(bool)));
    m_keyReference->registerShortcut(ant_Skl_Action);
    annotatorMenu->addAction(ant_Skl_Action);
    m_rightButtonTransformsMenu->addAction(ant_Skl_Action);
    //
    // 2
    //
    QAction *antr_t_Action = new QAction(tr("Transform"), this);
    antr_t_Action->setStatusTip(tr("Apply transform described in transform file <T> to all input audio files"));
    antr_t_Action->setShortcut(tr("Ctrl+H"));
    connect(antr_t_Action, &QAction::triggered , this, &MainWindow::transfom_t_file);
    //connect(this, SIGNAL(canAddLayer(bool)), antrAction, SLOT(setEnabled(bool)));
    m_keyReference->registerShortcut(antr_t_Action);
    annotatorMenu->addAction(antr_t_Action);
    m_rightButtonTransformsMenu->addAction(antr_t_Action);
    //


    // JPMAUS create new list of transforms that estimate Tempo
    // JPMAUS Theory: Whenever a (Notes) Layer or Transform added .. Use (TimeInstant) layer to calculate tempo .. This will be
    // JPMAUS done silently behind the scene.

    for (auto _trfm = m_transformActions.begin() ; _trfm != m_transformActions.end() ; _trfm++){
        if ( (*_trfm).second.contains("Tempo",Qt::CaseInsensitive) ){ // JPMAUS an abstract filter based on name
            bool _tempo_Trsfm_Added=false;
            if((*_trfm).second.endsWith("Tempo",Qt::CaseInsensitive) || (*_trfm).second.endsWith("Beats",Qt::CaseInsensitive)){
                // JPMAUS avoid duplicates ..
                for (auto itr = tempo_TransformActions.begin() ; itr != tempo_TransformActions.end() ; itr ++){
                    if ( (*itr).second == (*_trfm).second ){
                        _tempo_Trsfm_Added = true;
                        break;
                    }
                }
                if(!_tempo_Trsfm_Added) {tempo_TransformActions.push_back(*_trfm);}
                _tempo_Trsfm_Added=false;
            }
        }
    }
    TransformActions::reverse_iterator _tempo_Txfrm = tempo_TransformActions.rbegin();
    qDebug() << "List of Installed Tempo Transforms: " << endl;
    while (_tempo_Txfrm != tempo_TransformActions.rend()) {
        qDebug() << "Transform: " << _tempo_Txfrm->second << endl;
        _tempo_Txfrm++;
    }
    //
...
MainWindow::exportLayer()
{
    ...
    std::cout << "JPMAUS MainWindow::exportLayer(), Layer ModelID : " << modelId << std::endl;
    if (modelId.isNone()) return;
    std::cout << "JPMAUS , MainWindow::exportLayer() , Is Layer ModelID is None  : " << modelId.isNone() << std::endl;
...
MainWindow::addLayer()
{
    qDebug() << "JPMAUS Begin of Transform ...";
    QObject *s = sender();
    QAction *action = dynamic_cast<QAction *>(s);
    ...
    ExistingLayerActions::iterator ei = m_existingLayerActions.begin(); // JPMAUS typedef std::vector<std::pair<QAction *, Layer *>> ExistingLayerActions;
    while (ei != m_existingLayerActions.end()) {
        if (ei->first == action) break;
        ++ei;
    }

    // JPMAUS (1) .. Check if action is for existing layer .. if so .. just duplicate the layer
    if (ei != m_existingLayerActions.end()) {
        qDebug() << "JPMAUS .. Yes duplicating layer if transform was already done..!";
    ...
    // JPMAUS (2) check if requested action is in slice actions
    ...
    // (3) JPMAUS verify action for transform
    ...
    */
    ...
    QString transformId = i->second;    // JPMAUS ... i is TransformActions::iterator , defined above ..

    qDebug() << " JPMAUS Creating Transform : " << transformId;
    qDebug() << "JPMAUS Selected action for transform: " << i->first->text() << " ../ " << i->first->statusTip();
...
{
    qDebug() << "JPMAUS Transform starts ... for transformId " << transformId;
    Pane *pane = m_paneStack->getCurrentPane();
    if (!pane) {
        cerr << "WARNING: MainWindow::addLayer: no current pane" << endl;
        return;
    ...
    Transform transform;
    // JPMAUS #1# get Transformer for plugin
    try {
        transform = TransformFactory::getInstance()->
                getDefaultTransformFor(transformId);
        qDebug() << "JPMAUS Transform type : " << transform.getType();
    ...
    // JPMAUS #2#  Verify Base layer is Waveform
    ModelId defaultInputModelId;
    qDebug() << "JPMAUS Number of layers before Transform: " << pane->getLayerCount();
    for (int j = 0; j < pane->getLayerCount(); ++j) {
        Layer *layer = pane->getLayer(j);
        if (!layer) continue;
        ...
        qDebug() << "JPMAUS .. layer type: " << LayerFactory::getInstance()->getLayerTypeName(LayerFactory::getInstance()->getLayerType(layer)) ;
        ModelId modelId = layer->getModel();
        //
    ...
    //
    auto modelT = ModelById::get(defaultInputModelId);
    qDebug() << "JPMAUS defaultInputModelId is: " << modelT->getTypeName();
    //
    TransformUserConfigurator configurator;
    // JPMAUS This will, at some point, invoke transform cofgiuration menu!
    ...
    auto model = ModelById::get(input.getModel());
    //
    ...
    qDebug() << "JPMAUS ((1)) Adding OUTER layer  ";
    qDebug() << "------------------------------------------------------------------------------------------------------";
    try {
        ...
        if (newLayer) {
            ...
            //
            // JPMAUS here .. add Tempo calculations
            //
            auto nm = ModelById::getAs<NoteModel>(newLayer->getModel());
            bool isNM = true;
            if (!nm) {isNM = false;}
            //
            // JPMAUS .. Check if Transform is creating Notes Layer -- as SV stores MIDI files only for Note Layers
            bool isNotesLayer = LayerFactory::getInstance()->getLayerTypeName(
                        LayerFactory::getInstance()->getLayerType(newLayer)).contains("Note",Qt::CaseInsensitive);
            if (!isNM ){
                qDebug() << " JPMAUS .. No , not NotesLayer ..!"   ;
                //layer2Notes(newLayer);
                // JPMAUS Continue Normal
            } else if (isNotesLayer && configurator.includeTempoTransform==true) {  // Transform Setup Dialog --> configurator.includeTempoTransform
                qDebug() << "JPMAUS Yes! Notes layer being added ..!";
                updateMenuStates();
                // JPMAUS addLayer Tempo estimates
                TransformActions::reverse_iterator _tempo_Txfrm = tempo_TransformActions.rbegin(); // get last transform for tempo
                static QString tempoTransformId;
                for (; _tempo_Txfrm < tempo_TransformActions.rend(); ){
                    if(! _tempo_Txfrm->second.contains("aubio",Qt::CaseInsensitive)){ // JPMAUS preselection of aubio tempo
                        _tempo_Txfrm++;
                        continue;
                    }
                    tempoTransformId = _tempo_Txfrm->second;
                    break;
                }
                // Dialog : Tempo Transform selection
                QDialog* tempoDialog = new QDialog(this);
                static QHBoxLayout* dialogLayout = new QHBoxLayout;
                static QComboBox* txmSelector = new QComboBox(tempoDialog);
                txmSelector->clear();
                static QLabel* lbl = new QLabel("Select Tempo Transform");
                for (auto txr :  tempo_TransformActions ){
                    txmSelector->addItem(txr.second);
                }
                dialogLayout->addWidget(lbl);
                dialogLayout->addWidget(txmSelector);

                tempoDialog->setModal(true);
                tempoDialog->setWindowModality(Qt::ApplicationModal);
                //tempoDialog->setAttribute(Qt::WA_DeleteOnClose);
                ...
                tempoDialog->setWindowTitle(QCoreApplication::translate("Tempo Transform For Layer", "Tempo Transform For Layer", nullptr));

                QObject::connect(buttonBox, &QDialogButtonBox::accepted, tempoDialog, &QDialog::accept);
                QObject::connect(buttonBox, &QDialogButtonBox::rejected, tempoDialog, &QDialog::reject);

                static QVBoxLayout* vLayout = new QVBoxLayout;
                vLayout->addItem(dialogLayout);

                vLayout->addWidget(buttonBox);
                tempoDialog->setLayout(vLayout);

                if (tempoDialog->exec() == QDialog::Accepted){
                    tempoTransformId = txmSelector->currentText();
                } else {
                    return;
                }
                if(tempoTransformId.isEmpty() || tempoTransformId.isNull()){
                    tempoTransformId = tempo_TransformActions.rbegin()->second;
                    if(tempoTransformId.isNull() || tempoTransformId.isEmpty()){
                        QMessageBox::warning(this, tr("Tempo") , tr("Could not find installed Aubio plugin"),QMessageBox::Ok);
                        return;
                    }
                }
                // dialog
                qDebug() << "Selected Tempo Transform: " << tempoTransformId;
                // Emit signal to add inner layer
                QObject* context = new QObject;
                // Bug fix ... the layer model my get ready very fast ..!
                if(!nm->isReady()){
                    connect(newLayer , &Layer::modelChanged , context , [this ,context](ModelId mID){
                        delete context; // disconnect
                        Q_UNUSED(mID)
                        qDebug() << "JPMAUS .. Outer complete .. signal start Inner ";
                        emit startInnerTransform(tempoTransformId);
                    }, Qt::UniqueConnection);
                } else {
                    emit startInnerTransform(tempoTransformId);
                }
...
// JPMAUS Adding new method addInnerCalcsLayer()
// JPMAUS Added for 2 reasons (1) Eliminate Transform Configuration Dialog for Tempo / Silent Transform (2) emit a signal to merge with calling layer
// JPMAUS The caller needs the added layer to extract Tempo
// JPMAUS Transform with no user dialog!
void
MainWindow::addInnerCalcsLayer(const QString& tempoTransformId) {

    auto pane = m_paneStack->getCurrentPane();
    if (!pane) {
        cerr << "WARNING: MainWindow::addLayer: no current pane" << endl;
        return;
    }
    auto callingLayer = pane->getTopLayer();
    auto modelT = ModelById::get(callingLayer->getModel());
    qDebug() << "JPMAUS Model is: " << modelT->getTypeName();
    qDebug() << "JPMAUS Tempo Transform starts ... for transformId " << tempoTransformId;
    Transform transform;
    try {
        transform = TransformFactory::getInstance()->
                getDefaultTransformFor(tempoTransformId);
    } catch (std::exception &e) { // e.g. Piper server failure
        QMessageBox::critical
                (this, tr("Failed to query transform attributes"),
                 tr("<b>Failed to query transform attributes</b><p>Plugin or server error: %1</p>")
    ...
    for (int j = 0; j < pane->getLayerCount(); ++j) {
        ...
        qDebug() << "JPMAUS creating new Inner Calcs Layer  .. layer type: "
                 << LayerFactory::getInstance()->getLayerTypeName(LayerFactory::getInstance()->getLayerType(layer)) ;
        ModelId modelId = layer->getModel();
        if (modelId.isNone()) continue;
        for (ModelId candidateId : candidateInputModels) {
            if (candidateId == modelId) {
        ...
        // JPMAUS model For Inner?
        auto modelT = ModelById::get(modelId);
        qDebug() << "JPMAUS Inner Layer defaultInputModelId is: " << modelT->getTypeName();
        //
        if (!defaultInputModelId.isNone()) break;
    }
    ModelId aggregate;
    ...
    //
    // JPMAUS This is for transform cofgiuration menu .. we are skipping in Inner Transform!
    TransformUserConfigurator configurator;
    configurator.NoUserDialog=true; // JPMAUS Do not show Transform configuration Dialog
    //
    ModelTransformer::Input input = ModelTransformerFactory::getInstance()->
            getConfigurationForTransform
            (transform,
    ...
    auto model = ModelById::get(input.getModel());
    //
    qDebug() << "JPMAUS adding inner Layer ...1";
    try {
        Layer *tempoLayer = m_document->createDerivedLayer(transform, input);

        if (tempoLayer) {
            connect(tempoLayer , &Layer::modelChanged , this , &MainWindow::startCompoudTrasCals, Qt::UniqueConnection);
            //
            m_document->addLayerToView(pane, tempoLayer);
            m_document->setChannel(tempoLayer, input.getChannel());
            m_recentTransforms.add(tempoTransformId);
            m_paneStack->setCurrentLayer(pane, tempoLayer);
            // auto modelT = ModelById::get(tempoLayer->getModel());
            // qDebug() << "JPMAUS Model is: " << modelT->getTypeName();
            //
            updateMenuStates();
        }
...
void
MainWindow::compoundTransCalcs(ModelId mID){
    Q_UNUSED(mID)

    qDebug() << "Starting calcs slot!";
    auto pane = m_paneStack->getCurrentPane();
    Layer*  tempoLayer = pane->getTopLayer(); // Last added was Tempo
    if (tempoLayer == nullptr){
        QMessageBox::warning(this, tr("Merge Transformation") , tr("Could not find Tempo Transform"), QMessageBox::Ok);
        return;
    }
    disconnect(tempoLayer , &Layer::modelChanged , this , &MainWindow::startCompoudTrasCals);
    //auto modelTT = ModelById::get(tempoLayer->getModel());
    //qDebug() << "JPMAUS Model is: " << modelTT->getTypeName();

    // JPMAUS data is stored in model for the layer.
    auto tm = ModelById::getAs<SparseTimeValueModel>(tempoLayer->getModel());
    if(!tm) {
        qDebug() << tr("Internal error: unknown model");
        return;
    }
    while(! tm->isReady()){
        usleep(100000);
    ...
    //
    // JPMAUS Modify Outer Layer to include calculated Tempo
    // JPMAUS get the layer
    auto notesLayer = pane->getLayer(pane->getLayerCount()-2);
    //
    ModelDataTableModel *m_tableModel = new ModelDataTableModel(notesLayer->getModel());
    // JPMAUS Now loop the Table model and identify what columns it has!
    QStringList modelHeader{};
    int labelColumn=0;
    for (; labelColumn < m_tableModel->columnCount(); ++labelColumn) {
        QString ColHeadTitle = m_tableModel->headerData(labelColumn, Qt::Horizontal,Qt::DisplayRole).toString();
        modelHeader <<  ColHeadTitle;
        qDebug() << "Column: " <<  *modelHeader.rbegin();
        if (ColHeadTitle.contains("Label", Qt::CaseInsensitive)){
            break; //JPMAUS Ok , for now .. we only want Label column
        }
    }
    // JPMAUS Now we modify Label Column .. in short: use setData() on model .. with Qt::EditRole (so as to tell model to edit contents) ..
    // finally, emit singal dataChanged() .. (So that all consumers of model data like screnn or Layer->LayerData are updated with new content!)
    auto nm = ModelById::getAs<NoteModel>(notesLayer->getModel());
    if (!nm) {
        qDebug() << tr("Internal error: unknown model");
        return;
    }
    while(! nm->isReady()){
        qDebug() << "Waiting tones model ...";
        usleep(100000);
    }
    sv_frame_t last_Temp_Fr=0;
    EventVector tmEvents = tm->getAllEvents();
    auto itr = tmEvents.rbegin();
    while (itr != tmEvents.rend()){
        if(itr->hasValue()){
            last_Temp_Fr = itr->getFrame();
            qDebug() << "Last temp frame: " << last_Temp_Fr;
            break;
        }
        itr++;
    }
    ...
    for (const Event& notesEV : nm->getAllEvents()){
        sv_frame_t notesFrame = notesEV.getFrame();
        sv_frame_t onSetFrame=0;
        for (const Event& tempoEV : tm->getAllEvents()){
            //qDebug() << "Running Event ..." << notesEV.getValue();
            sv_frame_t tempoFrame = tempoEV.getFrame();
            if(  (notesFrame >= onSetFrame) &&  (notesFrame <= tempoFrame) ){
                QString BPM = QString::number(tempoEV.getValue() , 'f' ,2);
                nm->getSetDataCommand(looper,labelColumn,QString("%1 bpm").arg(BPM) ,Qt::EditRole);
                onSetFrame = tempoFrame; // maintain a slot
                break; // the while loop
            } else if (notesFrame >= last_Temp_Fr){  // No frame synch
                QString BPM = QString::number(itr->getValue() , 'f' ,2);
                nm->getSetDataCommand(looper,labelColumn,QString("%1 bpm").arg(BPM) ,Qt::EditRole);
                onSetFrame = tempoFrame; // maintain a slot
                break; // the while loop
            }
    ...
    qDebug() << "Transformation complete!";
    // JPMAUS Remove Tempo layer .. we dont need it
    m_document->removeLayerFromView(pane,tempoLayer);
    //QTimer::singleShot(500, this, [this,&pane, &tempoLayer](){m_document->removeLayerFromView(pane, tempoLayer); });

}

void MainWindow::layer2Notes(Layer * aLayer)
{
...
//
// JPMAUS , methods for Annotator actions

void MainWindow::genSkelton()
{

    QDialog *d = new QDialog(this);
    d->setWindowTitle(tr("Transform Skelton"));

    QGridLayout *layout = new QGridLayout;
    d->setLayout(layout);

    layout->addWidget(new QLabel(tr("Select Transform:")),
                      0, 0);
    QComboBox *listCombo = new QComboBox;
    TransformList list = TransformFactory::getInstance()->getAllTransformDescriptions();
    for (const TransformDescription& descriptor : list){
        qDebug() << descriptor.identifier;
        listCombo->addItem(descriptor.identifier);
    }
    layout->addWidget(listCombo,1,0);

    layout->addWidget(new QLabel(tr("Or Enter name for Transform:")),
                      2, 0);
    QLineEdit* txrmName = new QLineEdit;
    layout->addWidget(txrmName,3,0);
    listCombo->setCurrentIndex(-1);
    connect(listCombo, QOverload<const QString &>::of(&QComboBox::currentIndexChanged),
            [=](const QString &text){ txrmName->setText(text); });
    //
    QDialogButtonBox *bb = new QDialogButtonBox(QDialogButtonBox::Ok |
                                                QDialogButtonBox::Cancel);
    layout->addWidget(bb, 4, 0);
    connect(bb, SIGNAL(accepted()), d, SLOT(accept()));
    connect(bb, SIGNAL(accepted()), d, SLOT(accept()));
    connect(bb, SIGNAL(rejected()), d, SLOT(reject()));
    ...
    if (d->exec() == QDialog::Accepted) {
        ...
        QString name = txrmName->text();
        if(txrmName->text().isEmpty()){
            name = listCombo->currentText();
        }
        if (name.isEmpty() || name.isNull()){
            QMessageBox::information(this,tr("Transform Name"), tr("Enter Valid Transform Name"), QMessageBox::Ok);
            return;
        }
        Transform transform =
                TransformFactory::getInstance()->getDefaultTransformFor(name);
        cout << "@prefix xsd:      <http://www.w3.org/2001/XMLSchema#> ." << endl
             << "@prefix vamp:     <http://purl.org/ontology/vamp/> ." << endl
             << "@prefix :         <#> ." << endl << endl;
        QString rdf = RDFTransformFactory::writeTransformToRDF(transform, ":transform");
        cout << rdf;
        auto home = QStandardPaths::writableLocation(QStandardPaths::HomeLocation);
        QDir homeDiR = QDir(home);
        QFile rdfFile;
        rdfFile.setFileName(homeDiR.filePath("rdfFile.ttl") );
        if (rdfFile.open(QIODevice::WriteOnly) ){
            QTextStream out(&rdfFile);
            out << "@prefix xsd:      <http://www.w3.org/2001/XMLSchema#> ." << endl
                << "@prefix vamp:     <http://purl.org/ontology/vamp/> ." << endl
                << "@prefix :         <#> ." << endl << endl;
            out << rdf;
            rdfFile.close();
            QMessageBox::information(this,tr("Transform "), QString("Skelton generated %1").arg(rdfFile.fileName()), QMessageBox::Ok);
        } else {
            QMessageBox::information(this,tr("Transform "), tr("Could not write to file"), QMessageBox::Ok);
...
void MainWindow::transfom_t_file()
{
    QDialog *d = new QDialog(this);
    d->setWindowTitle(tr("Apply Transform "));
    d->setMinimumWidth(450);

    QGridLayout *layout = new QGridLayout;
    d->setLayout(layout);
    //
    layout->addWidget(new QLabel(tr("Select Transform file/Skeltop:")),
                      0, 0);
    //
    QLineEdit* txrmFileName = new QLineEdit;
    layout->addWidget(txrmFileName,0,1);
    //
    layout->addWidget(new QLabel(tr("Or, Apply default Transform for:")),
                      1, 0);
    ...
    static QString* transformName = new QString();
    layout->addWidget(listCombo,1,1);
    connect(listCombo, QOverload<const QString &>::of(&QComboBox::currentIndexChanged),
            [&](const QString &text){ *transformName = text; });
    //
    layout->addWidget(new QLabel(tr("Audio Folder:")),
                      2, 0);
    //
    QLineEdit* audioDirLE = new QLineEdit;
    layout->addWidget(audioDirLE,2,1);
    //

    QToolButton *tButton = new QToolButton;
    layout->addWidget(tButton,2,2);
    //
    static QString targetAudioFldr;
    connect(tButton , &QToolButton::pressed , this , [&](){
        targetAudioFldr = QFileDialog::getExistingDirectory
                (this, tr("Choose directorty containing Audio"));

        if (targetAudioFldr == QString()) return;
        audioDirLE->setText(targetAudioFldr);
    });
    //
    layout->addWidget(new QLabel(tr("Select writer:")),
                      3, 0);

    QComboBox* writerCombo = new QComboBox;
    writerCombo->addItems(QStringList{"csv", "midi","jams","lab","rdf","audiodb"});
    layout->addWidget(writerCombo,3,1);
    //
    QDialogButtonBox *bb = new QDialogButtonBox(QDialogButtonBox::Ok |
                                                QDialogButtonBox::Cancel);
    ...
    if (d->exec() == QDialog::Accepted) {
        bool result;
        QString txrmTTLFile = txrmFileName->text();
        // If no skelton provided .. assume default for selected transform
        if (txrmTTLFile.isEmpty() || txrmTTLFile.isNull()){
            result = QProcess::startDetached("sonic-annotator" , QStringList()
                                             << "-d"
                                             << *transformName
                                             << "-w"
                                             << writerCombo->currentText()
                                             << "-r"
                                             << audioDirLE->text()
                                             );


            // otherwise .. a skelton file specified ..
        } else{
            result = QProcess::startDetached("sonic-annotator" , QStringList()
                                             << "-t"
                                             << txrmTTLFile
        ...
        if (result == true) {
            QMessageBox::information(this,tr("Transform Completed"), QString("Transform started for folder %1").arg(audioDirLE->text()) , QMessageBox::Ok);
        } else {
            QMessageBox::warning(this,tr("Transform Failure"), tr("Transform process Could not be started") , QMessageBox::Ok);
        }
...
void MainWindow::audio4rmImage()
{
    // given a BMP image .. will try generate and import a wav file
    // #include sound_io.h  from ARSS
    // qtARSS2 test01.bmp test02.wav -s -b 12.000 -f 16 -r 44100 -p 150.000 -max 19912.127
    //wav_out(FILE *wavfile, double **sound, int32_t channels, int32_t samplecount, int32_t samplerate, int32_t format_param);
    //convertImage("test01.bmp","test02.wav","-s",NULL,"200","20000","12.000","150.000",NULL,NULL,NULL,NULL,NULL,NULL,"16",NULL);
    //
    QDialog *d = new QDialog(this);
    d->setWindowTitle(tr("Import Image"));
    ...
    //
    //
    layout->addWidget(new QLabel(tr("Image file:")),
                      0, 0);
    //
    QLineEdit* imagePath = new QLineEdit;
    layout->addWidget(imagePath,0,1);
    //

    QToolButton *tButton = new QToolButton;
    layout->addWidget(tButton,0,2);
    //
    static QString sourceIamge;
    connect(tButton , &QToolButton::pressed , this , [&](){
        sourceIamge = QFileDialog::getOpenFileName(this,
                                                   tr("Open Image"), ".", tr("Image Files (*.bmp)"));

        if (sourceIamge == QString()) return;
        imagePath->setText(sourceIamge);
    });
    //
    // Settings
    //1 bits
    layout->addWidget(new QLabel(tr("Bits Per Sample")),
                      1, 0);
    QComboBox* bitsPerSampleCombo = new QComboBox;
    bitsPerSampleCombo->addItems({"8","16","32"});
    layout->addWidget(bitsPerSampleCombo,1,1);
    // 2 rate
    layout->addWidget(new QLabel(tr("Sample Rate")),
                      2, 0);
    QComboBox* sampleRateCombo = new QComboBox;
    sampleRateCombo->addItems({"44100","48000","88200","96000"});
    layout->addWidget(sampleRateCombo,2,1);
    //3 min Freq
    layout->addWidget(new QLabel(tr("Min Frequency")),
                      3, 0);
    QSpinBox* minFreqSpin = new QSpinBox;
    minFreqSpin->setMinimum(200);
    minFreqSpin->setMaximum(22000);
    minFreqSpin->setSingleStep(100);
    minFreqSpin->setValue(200);
    layout->addWidget(minFreqSpin,3,1);
    // 4 max freq
    layout->addWidget(new QLabel(tr("Max Frequency")),
                      4, 0);
    QSpinBox* maxFreqSpin = new QSpinBox;
    maxFreqSpin->setMinimum(200);
    maxFreqSpin->setMaximum(22000);
    maxFreqSpin->setSingleStep(100);
    maxFreqSpin->setValue(19500);
    layout->addWidget(maxFreqSpin,4,1);
    // 5 Bands
    layout->addWidget(new QLabel(tr("Bands Per Octave")),
                      5, 0);
    QComboBox* bandsPerOctaveCombo = new QComboBox;
    bandsPerOctaveCombo->addItems({"8","16","24", "32","40","48","56","64","72","80"});
    layout->addWidget(bandsPerOctaveCombo,5,1);
    // 6
    layout->addWidget(new QLabel(tr("Pixels Per Second")),
                      6, 0);
    QSpinBox* pixelsPerSecondSpin = new QSpinBox;
    pixelsPerSecondSpin->setMinimum(50);
    pixelsPerSecondSpin->setMaximum(2500);
    pixelsPerSecondSpin->setSingleStep(50);
    pixelsPerSecondSpin->setValue(150);
    layout->addWidget(pixelsPerSecondSpin,6,1);
    //
    QDialogButtonBox *bb = new QDialogButtonBox(QDialogButtonBox::Ok |
                                                QDialogButtonBox::Cancel);
    layout->addWidget(bb, 7, 2);
    bb->setEnabled(false);
    connect(imagePath, &QLineEdit::textChanged, [bb]( const QString& text){
        if(text.isEmpty()) {return ;}
        QStringList fastCheck = text.split(".");
        if(fastCheck.length()<2) {bb->setEnabled(false); return;}
        if(fastCheck.last() == "bmp" || fastCheck.last() == "BMP")
            bb->setEnabled(true);
        else
            bb->setEnabled(false);
    });
    ...
    if (d->exec() == QDialog::Accepted) {
        ...
        sourceIamge = imagePath->text();
        if (sourceIamge.isEmpty() || sourceIamge.isNull()){
            QMessageBox::information(this,tr("Transform Name"), tr("Enter Valid Transform Name"), QMessageBox::Ok);
            return;
        }
        //convertImage("/tmp/test01.bmp","/tmp/tempwav4SV.wav","-s","44100","200",NULL,"32.000","600.000",NULL,NULL,NULL,NULL,NULL,NULL,"32",NULL);
        convertImage(sourceIamge.toLocal8Bit().data(),
                     QString("/tmp/tempwav4SV.wav").toLocal8Bit().data(),
                     QString("-s").toLocal8Bit().data(),
                     sampleRateCombo->currentText().toLocal8Bit().data(),
                     QString::number( minFreqSpin->value()).toLocal8Bit().data(),
                     NULL,
                     bandsPerOctaveCombo->currentText().toLocal8Bit().data(),
                     QString::number( pixelsPerSecondSpin->value()).toLocal8Bit().data() ,
                     NULL,NULL,NULL,NULL,NULL,NULL,
                     bitsPerSampleCombo->currentText().toLocal8Bit().data(),
                     NULL);
        //
        FileOpenStatus status = openPath("/tmp/tempwav4SV.wav", ReplaceSession);

        if (status == FileOpenFailed) {
            emit hideSplash();
            QMessageBox::critical(this, tr("Failed to open file"),
                                  tr("<b>File open failed</b><p>File \"%1\" could not be opened").arg("/tmp/tempwav4SV.wav"));
        } else if (status == FileOpenWrongMode) {
            emit hideSplash();
            QMessageBox::critical(this, tr("Failed to open file"),
                                  tr("<b>Audio required</b><p>Unable to load layer data from \"%1\" without an audio file.<br>Please load at least one audio file before importing annotations.").arg("/tmp/tempwav4SV.wav"));
        }
