...
public:
    ...
    virtual ~MainWindow();
// JPMAUS  define private member bool to bypass recursive Transformation // We will use this to create temporary transformation.
private:
    bool isTransformCallBack=false;
//
signals:
    void canChangeSolo(bool);
    void canAlign(bool);
    // JPMAUS  .. Primary Transform will emit signal to add inner transform
    void startInnerTransform(const QString&);
    // JPMAUS  Inner Transform emmits signal when finished .. calculations slot will pick up
    void startCompoudTrasCals(ModelId);
...
protected slots:
    // JPMAUS  cals Slot for Comppund Transform
    virtual void compoundTransCalcs(ModelId);
    virtual void layer2Notes(Layer*);
    // JPMAUS  Annotator slots
    virtual void genSkelton();
    virtual void transfom_t_file();
    //
    // JPMAUS  ARSS slot
    virtual void audio4rmImage();
    //
    ...
    virtual void addInnerCalcsLayer(const QString&);
...
protected:
    ...
    // JPMAUS  special Transform list for tempo estimates
    TransformActions tempo_TransformActions;
